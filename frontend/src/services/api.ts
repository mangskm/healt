...
export async function getMonthlyReport(month?: string): Promise<MonthlyReport> { const suffix = month ? `?month=${encodeURIComponent(month)}` : ""; const response = await apiFetch(`${apiBaseUrl}/api/v1/reports/monthly${suffix}`); if (!response.ok) throw new ApiError(response.status, "Report could not be loaded."); return response.json(); }
export async function downloadExport(dataType: ExportDataType, range: ExportRange, month?: string, start?: string, end?: string): Promise<void> { const params = new URLSearchParams({ range }); if (range === "month" && month) params.set("month", month); if (range === "custom") { if (start) params.set("start", start); if (end) params.set("end", end); } const response = await apiFetch(`${apiBaseUrl}/api/v1/exports/${dataType}.csv?${params.toString()}`); if (!response.ok) throw new ApiError(response.status, "CSV could not be prepared."); const blob = await response.blob(); const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = `${dataType}.csv`; document.body.appendChild(link); link.click(); link.remove(); URL.revokeObjectURL(url); }
