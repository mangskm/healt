...
class WeightRecordService:
    ...
    def list_records(self, limit: int, offset: int) -> WeightRecordListResponse:
        records, total = self._records.list_for_user(self._user.id, limit, offset)
        latest = self._records.latest_for_user(self._user.id)
    ...
    def get_record(self, record_id) -> WeightRecordResponse:
        record = self._records.get_for_user(self._user.id, record_id)
        if record is None:
    ...
    def create_record(self, payload: WeightRecordCreate) -> WeightRecordResponse:
        record = WeightRecord(user_id=self._user.id, weight_kg=to_kilograms(payload.weight, payload.unit), recorded_at=payload.recorded_at, note=payload.note)
