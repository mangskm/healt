...
class DashboardService:
    ...
    def get_dashboard(self, now: datetime | None = None) -> DashboardResponse:
        ...
        latest = self._weights.latest_for_user(self._user.id)
        goals = list(self._db.scalars(select(Goal).where(Goal.user_id == self._user.id, Goal.status == GoalStatus.ACTIVE).order_by(Goal.created_at.desc())))
        meals = list(self._db.scalars(select(Meal).options(selectinload(Meal.items)).where(Meal.user_id == self._user.id, Meal.eaten_at >= start, Meal.eaten_at < end).order_by(Meal.eaten_at.desc())))
        sessions = list(self._db.scalars(select(ExerciseSession).where(ExerciseSession.user_id == self._user.id, ExerciseSession.performed_at >= start, ExerciseSession.performed_at < end).order_by(ExerciseSession.performed_at.desc())))
        ...
        today_reminders = ReminderService(self._db, self._user).today(current).reminders
