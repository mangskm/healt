...
- Reports and CSV exports are read-only, user-scoped views of entered records. A report month and custom export dates are local calendar boundaries in the Profile timezone (UTC fallback), converted to UTC only for queries. Generate CSV server-side, retain missing optional nutrition as blank, and escape formula-like free-text cells before spreadsheet export. Do not add report snapshots, export history, generated-file persistence, recommendations, or medical interpretation without separately approved scope.
