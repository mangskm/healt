...
## Post-Roadmap UI shell

Post-Roadmap Iteration 1 keeps the frontend feature/API split intact while adding a shared authenticated application shell. Desktop navigation is a persistent sidebar; mobile navigation exposes core destinations plus a keyboard-accessible More menu. Shared frontend providers supply non-persistent success feedback and a deliberate confirmation dialog for destructive actions. These presentation helpers do not change backend contracts or persistence.
