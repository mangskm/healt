...
Post-Roadmap Iteration 2 backend tests cover Reports authentication and ownership, Profile-timezone month boundaries (including UTC/Asia-Bangkok date changes), empty/single/multiple Weight summaries and recorded change, direct Nutrition/Exercise totals, CSV content headers, local timestamps, custom-range validation, UTF-8 BOM/Thai text, blank nullable nutrition fields, and formula-injection mitigation. Frontend tests cover the Reports protected route, desktop/mobile navigation entry, month navigation, summary/empty/error states, export controls, and custom-range download request. Run the same full backend and frontend commands above; no migration is expected for reports.
